...
from .settings import get_screener_config, load_config
...
class StockCandidate:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "turnover": round(self.turnover, 3),      # 保留3位小数（亿元）
            "turnover_rate": round(self.turnover_rate, 2),  # 保留2位小数（%）
            "amplitude": round(self.amplitude, 2),    # 保留2位小数（%）
            "circ_mv": round(self.circ_mv, 2),        # 保留2位小数（亿元）
...
def _scan_from_hist_candidates(
    ...
    for row in rows:
        ...
        highs = [_num(x, ["最高"], 0.0) for x in records]
        lows = [_num(x, ["最低"], 0.0) for x in records]
        ...
        # 从实时数据行中提取成交金额、换手率、振幅和流通市值
        # 实时数据接口字段可能包括：成交额(amount)、换手率(turnover_rate)、振幅(amplitude)、流通市值(circ_mv)
        turnover = _num_unit(row, ["成交额", "amount", "成交额(元)", "成交金额"], 0.0) / 100_000_000  # 转换为亿元
        turnover_rate = _num(row, ["换手率", "turnover_rate", "turnover", "换手"], 0.0)
        amplitude = _num(row, ["振幅", "amplitude", "振幅%"], 0.0)
        circ_mv = _num_unit(row, ["流通市值", "circ_mv", "circ_market_value", "流通市值(元)"], 0.0) / 100_000_000  # 转换为亿元
        
        # 振幅备选计算：如果akshare未提供振幅数据（≈0），则手动计算
        if amplitude <= 0.1:  # 视为缺失或无效
            # 尝试从实时数据行获取最高价、最低价
            high_today = _num(row, ["最高", "high", "最高价"], 0.0)
            low_today = _num(row, ["最低", "low", "最低价"], 0.0)
            
            # 如果实时数据缺失，使用历史记录中的最新数据
            if high_today <= 0.1 or low_today <= 0.1:
                if len(highs) > 0:
                    high_today = highs[-1]
                if len(lows) > 0:
                    low_today = lows[-1]
            ...
            # 使用前收盘价计算振幅
            prev_close = closes[-2] if len(closes) > 1 else opens[-1] if len(opens) > 0 else 0.0
            if prev_close > 0 and high_today > 0 and low_today > 0:
                calculated_amplitude = ((high_today - low_today) / prev_close) * 100
                if calculated_amplitude > 0.1:  # 有效振幅
                    amplitude = calculated_amplitude
                    if debug:
                        debug_info.setdefault("amplitude_calc", []).append({
                            "code": code,
                            "calc_amp": round(calculated_amplitude, 2),
                            "source": "manual"
                        })
...
def scan_strong_stocks(
    ...
    # 调整系数：macro_weight=1（宏观极好）→ 放宽阈值（乘以 0.8）；macro_weight=0（宏观极差）→ 收紧阈值（乘以 1.2）
    adjust_factor = 1.2 - (macro_weight * 0.4)  # 1.2 ~ 0.8 线性变化
    if debug:
        print(f"[DEBUG] macro_weight={macro_weight}, adjust_factor={adjust_factor}")
    
    # 复制 screener 配置，避免修改原始配置（screener_cfg 已经是 screener 配置）
    screener = screener_cfg.copy()
    # 调整百分比类阈值
    pct_keys = ["min_change_pct", "prefilter_change_pct", "high_volume_bearish_drop_pct"]
    for key in pct_keys:
        if key in screener:
            screener[key] = round(screener[key] * adjust_factor, 2)
    
    # 调整量比阈值
    if "min_volume_ratio" in screener:
        screener["min_volume_ratio"] = round(screener["min_volume_ratio"] * adjust_factor, 2)
    
    # 调整趋势回看天数（宏观好时多看几天）
    if "trend_lookback" in screener:
        original = screener["trend_lookback"]
        screener["trend_lookback"] = max(2, min(6, int(original + (macro_weight * 2) - 0.5)))
    
    # 使用调整后的 screener 配置
    screener_cfg = screener
    ...
    if analysis_date and analysis_day != datetime.now().date():
        ...
        ranked = _scan_from_hist_candidates(zt_rows, analysis_ymd, sectors=sectors, top_n=top_n, debug_info=dbg, screener_cfg=screener_cfg, historical_mode=True, debug=debug)
        if debug:
    ...
    # Filter by sector early if sectors list provided
    pre_filtered = []
    sector_filter_active = bool(sectors)
    for row in rows:
        code = _str(row, ["代码", "股票代码"], "")
        name = _str(row, ["名称", "股票名称"], "")
        ...
        if change_pct > float(screener_cfg.get("prefilter_change_pct", 4.5)) and code:
            # Sector filter: check if stock belongs to any target sector
            if sector_filter_active:
                stock_sector = _str(row, ["所属行业", "行业", "所属板块"], "UNKNOWN")
                if stock_sector not in sectors:
                    continue
            pre_filtered.append(row)

    pre_filtered.sort(key=lambda x: _num(x, ["涨跌幅"], 0.0), reverse=True)
    # Adjust universe size based on sector filtering: if sectors provided, maybe fewer stocks
    universe_size = 80 if sector_filter_active else 120
    universe = pre_filtered[:universe_size]
    ranked = _scan_from_hist_candidates(universe, analysis_ymd, sectors=sectors, top_n=top_n, debug_info=dbg, screener_cfg=screener_cfg, historical_mode=False, debug=debug)
